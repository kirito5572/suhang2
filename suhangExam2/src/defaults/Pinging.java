...
public class Pinging extends Thread {
    ...
    private int portThreadCount = 10;
	...
	public Pinging(String ip, Object[] msg, int ThreadCount) {
		this.ip = ip;
		this.msg = msg;
		...
		try {
			...
			new Thread(() -> {
				if (msg[1] != "[n/a]" || msg[2] != "[n/s]") {
					final ExecutorService es = Executors.newFixedThreadPool(portThreadCount);
					final int timeout = 200;
					final List<Future<ScanResult>> futures = new ArrayList<>();
					
					for (int port = 1; port <= 1024; port++) {
						futures.add(PortScanner.portIsOpen(es, ip, port, timeout));
					}
					try {
